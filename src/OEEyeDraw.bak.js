...
var DoodlePopup = (function() {
	...
	/**
	 * [DoodlePopup description]
	 * @todo Use mustache templates to render and re-render the popup.
	 * @param {[type]} drawing         [description]
	 * @param {[type]} widgetContainer [description]
	 */
	function DoodlePopup(drawing, widgetContainer) {
		...
		this.deleteButton = this.container.find('.icon-ed-delete').closest('.eyedraw-button');
...
ED.init = function(_properties) {
	...
	var drawingInstance = new ED.Drawing(canvas, _properties.eye, _properties.idSuffix, _properties.isEditable, drawingOptions);
	var toolbar = new ED.Toolbar(drawingInstance, container);
	var doodlePopup = new ED.DoodlePopup(drawingInstance, _containerElement);
